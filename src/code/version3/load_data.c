...
void fill_missing_values(struct point **, int);
...
/* 		are set.		 				*/
/************************************************************************/
int load_points(infile, points_ptr) FILE *infile;
POINT ***points_ptr;
{
  ...
  while (TRUE) {
    ...
    if (point_count == 1 && no_of_dimensions == MAX_NO_OF_ATTRIBUTES)
      ...
      category value, unless unlabeled is TRUE. */
    {
      ...
      while (TRUE) {
          ...
          if (temp != HUGE) {
              ...
              if (categories_unknown)
                ...
                for (i = 1; i <= no_of_categories; i++)
                  if (temp == category_array[i]) {
                    array_name[i]->category = i;
                    break;
                  } else if (i < 1 || i > no_of_categories) {
                    printf(
                        "Load_Points: Classes should be integers in [1,%d].\n",
                        no_of_categories);
                    error("");
                  }
            }
            ...
            for (i = 1; i <= points_allocated; i++) {
              ...
              array_name[i]->dimension = (float *)realloc(
                  array_name[i]->dimension, no_of_dimensions * sizeof(float));
              array_name[i]->dimension -= 1;
            }
            break;
          }
        ...
        if (isalpha(c)) error("Load_Points: Alphabetic character in datafile.");
        if (temp != HUGE) {
      ...
      for (j = 1; j <= no_of_dimensions; j++) {
        ...
        if (c == '?') {
          ...
          if (i != 1) {
            if (j > 1)
              error("Load_Points : Object with too few attributes.");
            else
              break;
          }
        }
      ...
      if (categories_unknown) {
        ...
        if (j > no_of_categories) {
          ...
          category_array += 1;
          category_array =
              (int *)realloc(category_array, no_of_categories * sizeof(int));
          category_array -= 1;
  ...
  if (point_count != points_allocated)
    array_name =
        allocate_point_array(array_name, point_count, points_allocated);
  ...
  if ((!unlabeled && categories_unknown && training_set) ||
    ...
    for (i = 1; i <= no_of_categories; i++)
      if (category_array[i] < 1 || category_array[i] > no_of_categories) break;
...
/************************************************************************/
POINT **allocate_point_array(array_name, size, prev_size) POINT **array_name;
int size, prev_size;
{
  int i;

  if (prev_size == 0) {
    if (array_name != NULL) free((char *)(array_name + 1));

    array_name =
        (struct point **)malloc((unsigned)size * sizeof(struct point *));
    if (!array_name)
...
/************************************************************************/
shuffle_points(array_name, count) POINT **array_name;
int count;
{
  int i, newposition;
  POINT *temp_point;
...
/*                            Defined in oc1.h.                         */
/************************************************************************/
void fill_missing_values(points, no_of_points) struct point **points;
int no_of_points;
{
  int i, j, count;
