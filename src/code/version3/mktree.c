...
char *argv[];
{
  ...
  while ((c1 = getopt(argc, argv,
                      "aA:b:Bc:d:D:i:j:Kl:m:M:n:Nop:r:R:s:t:T:uvV:")) != EOF)
    ...
    switch (c1) {
      ...
      case 'A': /*File into which all the locations of the
                                    ...
                                    animation of the tree building process,
                   using this output. */
...
allocate_structures(no_of_points) int no_of_points;
{
  ...
  candidates =
      (struct unidim *)malloc((unsigned)no_of_points * sizeof(struct unidim));
  candidates -= 1;
...
/* Remarks :                                                            */
/************************************************************************/
struct tree_node *build_tree(points, no_of_points,
                             dt_file) struct point **points;
int no_of_points;
char *dt_file;
{
...
/*				node is created.			*/
/************************************************************************/
struct tree_node *build_subtree(node_str, cur_points,
                                cur_no_of_points) char *node_str;
POINT **cur_points;
...
char *cur_label;
{
  ...
  while (TRUE) {
    ...
    for (cur_coeff = 1; cur_coeff < no_of_coeffs; cur_coeff++) {
      new_error =
          cart_perturb(cur_points, cur_no_of_points, cur_coeff, cur_error);
    ...
    if (cur_error != 0) {
      new_error =
          cart_perturb_constant(cur_points, cur_no_of_points, cur_error);
      if (alter_coefficients(cur_points, cur_no_of_points)) {
        if (veryverbose)
          printf("\tCART hill climbing for coeff. %d. impurity %.3f -> %.3f\n",
...
/* Is called by modules :	build_subtree				*/
/************************************************************************/
float oblique_split(cur_points, cur_no_of_points, cur_label) POINT **cur_points;
int cur_no_of_points;
char *cur_label;
{
  char c;
    ...
    if (order_of_perturbation == RANDOM) {
      ...
      for (i = 1; i <= cycle_count; i++) {
        ...
        } else /*Try improving in a random direction*/
        {
          ...
          j = 0;
          while (cur_error != 0 && !improved_in_this_cycle &&
                 ++j <= max_no_of_random_perturbations) {
            new_error =
                perturb_randomly(cur_points, cur_no_of_points, cur_error);
            if (alter_coefficients(cur_points, cur_no_of_points)) {
              if (veryverbose)
  ...
  for (i = 1; i <= no_of_coeffs; i++) coeff_array[i] = best_coeff_array[i];
  coeff_modified = TRUE;
  find_values(cur_points, cur_no_of_points);
  set_counts(cur_points, cur_no_of_points, 1);
  return (least_error);
...
/* Is called by modules :	build_subtree				*/
/************************************************************************/
float axis_parallel_split(cur_points, cur_no_of_points) POINT **cur_points;
int cur_no_of_points;
{
...
POINT **points;
{
  ...
  clock_t begin, end;
  char result_string[4096], line[4096];

  results = (struct test_outcome *)malloc((unsigned)no_of_folds *
                                          sizeof(struct test_outcome));
  ...
  *result_string = 0;
  while (TRUE) {
    ...
    results[no_of_folds] =
        estimate_accuracy(test_points, no_of_test_points, root);
    end = clock();
    time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
    results[no_of_folds].leaf_count = leaves_count(root);
    printf(
        "fold %d: acc. = %.2f\t#leaves=%.0f\t#nodes=%.0f\tmax. "
        "depth=%.0f\n",
        no_of_folds, results[no_of_folds].accuracy,
        results[no_of_folds].leaf_count, results[no_of_folds].node_count,
        results[no_of_folds].tree_depth);
    sprintf(line, "***%d***%.7f***%.7f***\n", no_of_folds,
            results[no_of_folds].accuracy / 100, time_spent);
    strcat(result_string, line);
  ...
  resultsum.leaf_count = resultsum.node_count = resultsum.tree_depth =
      resultsum.accuracy = 0;
  ...
  if (verbose) printf("\nOverall:\t");
  printf("accuracy=%.2f\n#leaves=%.2f\n#nodes=%.2f\nmax depth=%.2f\n",
         resultsum.accuracy, resultsum.leaf_count, resultsum.node_count,
         resultsum.tree_depth);
...
print_log_and_exit() {
  ...
  if (strlen(animation_file) && no_of_dimensions == 2) {
    ...
    fprintf(logfile,
            "Use the Display program with -A option to see animation.\n");
  }
