...
char *argv[];
{
  ...
  if (argc == 1)
    usage(pname);
  while ((c1 =
              getopt(argc, argv, "aA:b:Bc:d:D:i:j:Kl:m:M:n:Nop:r:R:s:t:T:uvV:")) != EOF)
    ...
    case 'j': /*Maximum number of random perturbations tried when
      ...
      if (max_no_of_random_perturbations < 0)
        usage(pname);
      break;
  ...
  if (unlabeled == TRUE && !strlen(test_data))
    usage(pname);
  ...
  if (strlen(train_data))
  {
    ...
    {
      if (!strlen(dt_file))
        sprintf(dt_file, "%s.dt", train_data);
      root = build_tree(train_points, no_of_train_points, dt_file);
    }
    else
    {
      if (no_of_folds == -1)
        no_of_folds = no_of_train_points;
...
struct tree_node *create_tree_node()
{
  ...
  if (cur_node == NULL)
    error("Create_Tree_Node : Memory allocation failure.");
...
POINT **points;
{
  ...
  *result_string = 0;
  while (TRUE)
  {
    ...
    if (no_of_points - fold_end < fold_size)
      fold_end = no_of_points;
    no_of_folds++;
    ...
    sprintf(line, "***%d***%.7f***%.7f***\n",
            no_of_folds,
            results[no_of_folds].accuracy / 100,
            time_spent);
