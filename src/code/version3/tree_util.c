...
struct tree_node *read_tree(decision_tree) char *decision_tree;
{
  ...
  if (!(read_header(dtree))) error("Decision tree invalid/absent.");

  if ((root = read_hp(dtree)) == NULL) error("Decision tree invalid/absent.");
...
/************************************************************************/
void read_subtree(root, dtree) struct tree_node *root;
FILE *dtree;
{
...
/************************************************************************/
struct tree_node *read_hp(dtree) FILE *dtree;
{
  ...
  for (i = 1; i < no_of_categories; i++)
    if (fscanf(dtree, "%d,", &cur_node->left_count[i]) != 1) return (NULL);
  if (fscanf(dtree, "%d], Right = [",
             &cur_node->left_count[no_of_categories]) != 1)
    return (NULL);
  for (i = 1; i < no_of_categories; i++)
    if (fscanf(dtree, "%d,", &cur_node->right_count[i]) != 1) return (NULL);
  ...
  for (i = 1; i <= no_of_categories; i++) {
    cur_node->no_of_points +=
        cur_node->left_count[i] + cur_node->right_count[i];
...
/*		your decision tree files are not produced by "mktree".	*/
/************************************************************************/
int read_header(dtree) FILE *dtree;
{
...
int leaves_count(struct tree_node *cur_node) {
  ...
  if (cur_node->left == NULL && cur_node->right == NULL) return 1;
...
/*				write_tree				*/
/************************************************************************/
void write_subtree(cur_node, dtree) struct tree_node *cur_node;
FILE *dtree;
{
  if (cur_node == NULL) return;
...
/*				structure of the DT.			*/
/************************************************************************/
void write_hp(cur_node, dtree) struct tree_node *cur_node;
FILE *dtree;
{
  int i;
...
/************************************************************************/
void write_header(dtree) FILE *dtree;
{
  extern int no_of_dimensions, no_of_categories;
