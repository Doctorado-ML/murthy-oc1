...
struct tree_node *root;
{
  ...
  int leaf_count(), tree_depth(), node_count();
  ...
  if (strlen(misclassified_data))
    outfile = fopen(misclassified_data, "w");
  else
    outfile = NULL;
